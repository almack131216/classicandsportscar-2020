...
}elseif($currentpage=="filmTV"){
	$query = "SELECT id,name,image_large,detail_2,detail_3,price FROM catalogue WHERE id=$itemID OR id_xtra=$itemID AND image_large!='' ORDER BY position_initem ASC";
}else{
...
if($result && mysql_num_rows($result)>=1){
	...
	for($i=0;$i<mysql_num_rows($result);$i++){
		...
		$tab_content .= '<div id="tab'.$rowNum.'" class="tab_content">'."\r\n";
			...
			$tmpSize = "homepage";
			if($currentpage=="filmTV") $tmpSize = "large";

			$attributes = array('filename'=>$row['image_large'],'size'=>$tmpSize,'alt'=>$row['name'],'class'=>"large");
			//$tab_content .= $linkTag;
